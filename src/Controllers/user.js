...
const verifyOTP = async function (req, res) {
  try {
    ...
    res.status(200)({ success: true, message: "OTP verified" });
  } catch (error) {
    return res.status(500).send({
      status: false,
      message: error.message,
...
const createInvites = async function (req, res) {
  try {
    ...
    //   console.log(phones, "aaaa");
    //   var sendMessage = `${SMS_GATEWAY_ENDPOINT}?key=${SMS_GATEWAY_API_KEY}&secret=${SMS_GATEWAY_API_SECRET}&from=${SMS_GATEWAY_SENDER}&to=${phones}&text=${message}`;

    //   await axios.get(sendMessage);

    //   data[i].coach_id = userId;
    //   let invitesCreated = await invitesModel.create(data[i]);
    //   feedbackArr.push(invitesCreated);
    // }
    // return res.status(201).send({
    //   status: true,
    //   message: "Success",
